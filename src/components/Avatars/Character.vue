<template>
  <div>
    <div class="character-position" :style="{width: this.characterPositionWidth}">
      <div v-on:click="selectYourCharacter" ref="characterImg" v-html="this.svg"
           :style="{width: this.width, height: this.height}" style="margin: 5px auto auto auto;"></div>
      <span v-if="isName"
            style="white-space: nowrap;overflow: hidden; text-overflow: ellipsis;margin-bottom: 0.5em;color: #848484;font-size: 14px;">{{
          this.svgColor.name
        }}</span>
...
export default {
  ...
  methods: {
     ...
     * @param {String} color
     * @param {Number} proportion of darker shade
     * @return none
     */
        ...
        if (![""].includes(classValue)) {
          ...
          if (classValue.includes(" ")) {
            ...
            for (let j = 0; j < classValue.length; j++) {
              ...
              characterImgSVG.children[i].classList.remove(classValue[j]);
            }
          }
