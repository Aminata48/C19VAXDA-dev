      ...
      <div class="modal-card mobile-modal">
        <header class="modal-card-head">
          <p class="modal-card-title">
            ...
            <span style="font-size: medium">{{
                this.labels.totalCharacterCount
              }} {{ this.currentCharacterNumber }} / {{ totalCharactersCount }}</span>
          </p>
        ...
        <section class="modal-card-body">
          <div class="columns">
            <div class="column is-centered ">
              <Character v-if="isActive" :size="{width: '70px', height: '78px'}"
                         :change-avatar-state="changeAvatarState" :edit="false" :customised="true" ref="character"
                         :id="'current'" :svgFile="this.currentCharacter"
              ...
              <v-btn v-if="isEdit" color="#212121" style="color:white" v-on:click="this.changeAvatar">
                {{ this.labels.changeAvatar }}
              </v-btn>
            ...
            <div class="column is-center">
              <div class="tabs is-centered is-boxed is-three-quarters">
                <ul>
                  ...
                  <li class="tab" id="hairColorTab" v-if="this.hasHair"
                      v-on:click="openTab('hairColorTab', 'hairColorSelect')">
                    <a>{{ this.labels.hairColorTab }}</a>
                  </li>
                  <li class="tab" id="glassesTab" v-if="this.hasGlasses"
                      v-on:click="openTab('glassesTab', 'glassesSelect')">
                    <a>{{ this.labels.glassesTab }}</a>
                  </li>
                  <li class="tab" id="facialHairTab" v-if="this.hasFacialHair"
              ...
              <div style="display: flex; justify-content: center">
                ...
                <div id="hairColorSelect" class="content-tab" v-if="this.hasHair">
                  ...
                  <div style="display: flex; justify-content: center;">
                    <v-btn v-if="!this.isVueColorActive" :disabled='this.showMoreColor' color="black"
                           style="margin: 10px; color:white" @click="showSpectrum">{{ this.labels.moreColor }}
                    </v-btn>
                    <v-btn v-if="this.isVueColorActive" color="black" style="margin: 10px; color: white"
                           @click="addColorToSpectrum">{{ this.labels.addColor }}
                    </v-btn>
                    <v-btn v-if="this.isVueColorActive" color="#D50000" style="margin: 10px; color: white"
                           @click="closeSpectrumColorCanvas">{{ this.labels.cancelSpectrumColor }}
                    </v-btn>
                  </div>
                ...
                <!-- Glasses -->
                <div id="glassesSelect" style="justify-content: center;" class="content-tab buttons"
                     v-if="this.hasGlasses">
                  <ul>
                    <li class="accessoriesList button" style="overflow: hidden" v-on:click="selectGlasses(-1)">
                      {{ this.labels.None }}
                    </li>
                    <li class="accessoriesList button" style="overflow: hidden" v-for="(glasses, index) in glassesList"
                        :key="index" v-on:click="selectGlasses(index)"
                        v-html="require(`@/assets/glasses/${glasses}.svg`)"></li>
                  </ul>
                </div>

                <!-- Facial hair -->
                <div id="facialHairSelect" style="justify-content: center;" class="content-tab buttons"
                     v-if="this.hasFacialHair">
                  <ul>
                    <li class="accessoriesList button" style="overflow: hidden" v-on:click="selectBeards(-1)">
                      {{ this.labels.None }}
                    </li>
                    <li class="accessoriesList button facialHairList" style="overflow: hidden"
                        v-for="(beard, index) in facialHairListJson" :key="index"
                        v-html="require(`@/assets/facialHair/${beard.file}`)" v-on:click="selectBeards(index)"></li>
                  </ul>
                </div>
              </div>
            </div>
        ...
        <footer class="modal-card-foot">
          <div class="buttons is-light">
            <button class="button is-success" v-on:click="this.saveCharacter">{{ this.labels.continueBtn }}</button>
...
export default {
  ...
  methods: {
    ...
    saveCharacter() {
      ...
      if (this.replaceCharacterMode) {
        this.saveCharacterAfterReplace(this.currentCharacterObject, this.$refs.character.getSvgColor(), this.characterType);
